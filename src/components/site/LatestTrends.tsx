import Link from "next/link";

import { formatDate } from "@/lib/utils";

type LatestTrendsProps = {
  dashboard: {
    snapshot: {
      sourceDate: Date;
      repos: Array<{
        id: string;
        repoFullName: string;
        starsGained: number | null;
        language: string | null;
      }>;
    };
    analytics: {
      totalRepos: number;
      totalStarsGained: number;
      topLanguages: Array<{ name: string; count: number }>;
    };
    summary: {
      aiSummary: string | null;
      editedSummary: string | null;
      status: string;
    } | null;
  } | null;
};

export function LatestTrends({ dashboard }: LatestTrendsProps) {
  if (!dashboard) {
    return (
      <section className="glass card">
        <p className="section-title">GitHub 趋势</p>
        <h2 className="mt-2 text-2xl font-semibold">暂时还没有抓取数据</h2>
        <p className="mt-3 text-[var(--muted)]">启动 worker 或在后台手动触发一次趋势抓取后，这里会出现摘要和分析。</p>
      </section>
    );
  }

  const summary = dashboard.summary?.editedSummary || dashboard.summary?.aiSummary;

  return (
    <section className="glass card">
      <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
        <div className="max-w-2xl">
          <p className="section-title">GitHub 趋势</p>
          <div className="mt-2 flex items-center gap-3">
            <h2 className="text-2xl font-semibold">最新趋势摘要</h2>
            <span className="text-sm text-[var(--muted)]">{formatDate(dashboard.snapshot.sourceDate)}</span>
          </div>
          <p className="mt-4 leading-8 text-[var(--muted)]">
            {summary || "本次抓取已经完成，但 AI 摘要还未生成。你仍然可以在趋势页查看原始排行榜和基础统计。"}
          </p>
          <div className="mt-5 flex flex-wrap gap-2">
            {dashboard.analytics.topLanguages.map((language) => (
              <span key={language.name} className="tag">
                {language.name} · {language.count}
              </span>
            ))}
          </div>
        </div>
        <div className="rounded-3xl border border-[var(--line)] bg-white/75 p-5 lg:w-[340px]">
          <p className="text-sm text-[var(--muted)]">本次抓取概览</p>
          <dl className="mt-4 grid grid-cols-2 gap-4">
            <div>
              <dt className="text-sm text-[var(--muted)]">仓库数</dt>
              <dd className="mt-1 text-2xl font-semibold">{dashboard.analytics.totalRepos}</dd>
            </div>
            <div>
              <dt className="text-sm text-[var(--muted)]">Star 增量</dt>
              <dd className="mt-1 text-2xl font-semibold">{dashboard.analytics.totalStarsGained}</dd>
            </div>
          </dl>
          <div className="mt-6 space-y-3">
            {dashboard.snapshot.repos.slice(0, 3).map((repo) => (
              <div key={repo.id} className="rounded-2xl border border-[var(--line)] px-4 py-3">
                <p className="font-medium">{repo.repoFullName}</p>
                <p className="text-sm text-[var(--muted)]">
                  {repo.language || "Unknown"} · +{repo.starsGained ?? 0} stars
                </p>
              </div>
            ))}
          </div>
          <Link href="/trending" className="btn btn-primary mt-6 w-full">
            查看完整趋势页
          </Link>
        </div>
      </div>
    </section>
  );
}
